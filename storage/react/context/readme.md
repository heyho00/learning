...
액션과 리듀서라는 개념을 사용해 상태 업데이트 로직을 컴포넌트 밖으로 분리할 수 있게 해주며, 상태가 업데이트 될 때 실제로 의존하고 있는 값이 변할때만 리렌더링 하도록 최저화 해준다.

context를 쓴다면, 각기 다른 상태마다 새롭게 context를 만들어줘야 하는데 이 과정을 생략하게 해줘 편리하다.
